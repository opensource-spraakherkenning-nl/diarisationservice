#!/bin/sh
set -e
docker build -t "diarisationservice" .
...
#At this point you can pass any environment variables you use in your diarisationservice.config.yml, pass them via --env
docker run --rm --volume "$DATADIR:/data" -p "$HOST_PORT:80" --env CLAM_ROOT=/data/diarisationservice-userdata --env CLAM_USE_FORWARDED_HOST=0 "diarisationservice"
